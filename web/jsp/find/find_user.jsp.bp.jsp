...
<script>
	...
	function ClickTable(mpara){
		...
		formCurrent.tUserID.value=document.getElementById("A"+mpara).innerText;
		formCurrent.tUserName.value=document.getElementById("B"+mpara).innerText;
...
<BR>
<div class="container">
<form name="form1" method="post"  action="<%=contextPath%>/jsp/find/find_user.jsp">
...
<div class="col-sm-12">
				  ...
				  <table width="100%">
				  <tr>
				  	<td align="center" bgcolor="#006699"><label><span style="color:#FFFFFF">User name</span></label></td>
			...
			<div class="table-responsive"> 
					<table cellspacing="2" class="table table-striped table-hover">
					  <tr>
						...
						sqland="";
						if(HFind.equals("1")){	
							sqland=" and username like '%"+txtuniv_name+"%' ";	
						}else{				
						}
						//-----------------------------Select Data from slf_institute------------------------------------------------------------	
						s1=s1.equals("")?"Y":s1;
						sql="select * from slf_sso.userlogin where 1=1  "+sqland+"  and active_flag='"+s1+"' order by userid";
						...
						<tr >
						  ...
						  <td align="center"   id="A<%=Integer.toString(i)%>" onClick="ClickTable('<%=Integer.toString(i)%>');" class="<%=bor1%>"><%=rs.getString("userid")%></td>
						  <td  align="left"  id="B<%=Integer.toString(i)%>" onClick="ClickTable('<%=Integer.toString(i)%>');"  class="<%=bor1%>"><%=rs.getString("username")%></td>
						</tr>			
						<%
						}
						rs.close();	stmt.close();							
